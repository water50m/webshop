...
from app.models import StocktakeLine, StocktakeSession, StocktakeStatus, User, UserRole
...
def list_sessions(db: Session = Depends(get_db)):
    return [_serialize_session(s) for s in db.query(StocktakeSession).order_by(StocktakeSession.opened_at.desc()).all()]


@router.get("/sessions/current", response_model=StocktakeSessionOut | None)
def current_session(db: Session = Depends(get_db)):
    session = stocktake_service.get_open_session(db)
...
def open_session(
    payload: OpenSessionIn = OpenSessionIn(), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    try:
        session = stocktake_service.open_session(db, user, note=payload.note)
...
def submit_line_count(session_id: int, line_id: int, payload: CountIn, db: Session = Depends(get_db)):
...
def close_session_endpoint(session_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    session = db.get(StocktakeSession, session_id)
    if session is None:
        raise HTTPException(status_code=404, detail="ไม่พบรอบนับสต๊อกนี้")
    try:
