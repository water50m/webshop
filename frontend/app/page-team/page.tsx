...
export default function PageTeamPage() {
  ...
  return <main className="w-full p-4 md:p-6">
    <div className="rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white p-5 shadow-sm">
      ...
      {channels.length ? <p className="mt-5 rounded-lg border border-indigo-200 bg-white px-3 py-2 text-sm text-slate-700">กำลังจัดการสิทธิ์ของเพจ: <span className="font-semibold text-slate-900">{channels[0].name || channels[0].page_id}</span></p> : <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><p className="font-medium">ยังไม่มี Facebook Page ที่เข้าถึงได้</p><p className="mt-1 text-amber-900">ให้เข้าสู่ระบบ Facebook อีกครั้งเพื่อตรวจสิทธิ์ แล้วเลือก Page เพื่อลงทะเบียนหรือเปิดใช้งาน</p><button type="button" onClick={() => void restartFacebookOnboarding()} disabled={busy} className="mt-3 inline-flex h-10 items-center gap-2 rounded-lg bg-[#1877f2] px-4 text-sm font-medium text-white hover:bg-[#166fe5] disabled:cursor-not-allowed disabled:opacity-60"><MessageCircle className="h-4 w-4" />ดำเนินการต่อด้วย Facebook</button></div>}
    </div>
